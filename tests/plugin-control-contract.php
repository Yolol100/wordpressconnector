...
foreach ($mustContain as $relative => $needles) {
    ...
    foreach ($needles as $needle) {
        if (strpos($source, $needle) === false) {
            fwrite(STDERR, "Missing plugin-control contract fragment in {$relative}: {$needle}\n");
            exit(1);
        }
    }
}
...
if ($settingsAdapter === false || $autoImageAdapter === false) {
    fwrite(STDERR, "Unable to read plugin settings adapters.\n");
    exit(1);
}

foreach (array('eval(', 'shell_exec(', 'passthru(', 'proc_open(', 'popen(') as $forbidden) {
    if (strpos($settingsAdapter, $forbidden) !== false || strpos($autoImageAdapter, $forbidden) !== false) {
        fwrite(STDERR, "Forbidden execution primitive in plugin settings bridge: {$forbidden}\n");
        exit(1);
    }
}
