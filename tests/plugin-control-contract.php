...
$mustContain = array(
    ...
    'docs/PLUGIN-CONTROL.md' => array('Broken Link Checker', 'Wordfence Security', 'WP Mail SMTP', 'Code Snippets', 'WP File Manager', 'WP File Manager and controlled filesystem access', 'WP Agent / approved authenticated HTTPS client -> WordPress Connector'),
    'docs/ACTION-CATALOG.md' => array('safe agent/connector control mode without exposing secrets'),
);
...
$pluginControl = file_get_contents($root . '/docs/PLUGIN-CONTROL.md');
$catalog = file_get_contents($root . '/docs/ACTION-CATALOG.md');
if ($settingsAdapter === false || $autoImageAdapter === false || $pluginControl === false || $catalog === false) { fwrite(STDERR, "Unable to read plugin settings control sources.\n"); exit(1); }
...
foreach (array('safe ChatGPT/GitHub control mode', 'exported through GitHub', 'never exported through GitHub') as $stale) {
    if (strpos($settingsAdapter, $stale) !== false || strpos($catalog, $stale) !== false || strpos($pluginControl, $stale) !== false) {
        fwrite(STDERR, "Retired GitHub live-transport wording remains: {$stale}\n");
        exit(1);
    }
}
if (strpos($pluginControl, 'ChatGPT -> GitHub -> WordPress Connector') !== false) {
    fwrite(STDERR, "Retired GitHub live-transport route remains in plugin control docs.\n");
    exit(1);
}
