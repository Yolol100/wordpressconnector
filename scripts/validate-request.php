...
if ('' === $raw || strlen($raw) > 262144) {
    fwrite(STDERR, "Request must be between 1 byte and 256 KiB.\n");
    exit(1);
}
...
$allowed = array('version','request_id','action','dry_run','confirm','expected_fingerprint','expected_state_token','payload');
foreach (array_keys($data) as $key) {
    if (! is_string($key) || ! in_array($key, $allowed, true)) $errors[] = 'Unknown top-level key: ' . (string) $key;
}
if (! array_key_exists('version', $data) || 1 !== $data['version']) $errors[] = 'version must be integer 1.';
$id = $data['request_id'] ?? null;
if (! is_string($id) || ! preg_match('/^[A-Za-z0-9][A-Za-z0-9._-]{7,99}$/D', $id)) $errors[] = 'Invalid request_id.';
$action = $data['action'] ?? null;
if (! is_string($action) || ! preg_match('/^[a-z0-9][a-z0-9._-]*$/D', $action)) $errors[] = 'Invalid action.';
...
foreach (array('expected_fingerprint','expected_state_token') as $guard) {
    if (! array_key_exists($guard, $data) || null === $data[$guard]) continue;
    if (! is_string($data[$guard]) || ! preg_match('/^[a-f0-9]{64}$/D', $data[$guard])) $errors[] = 'Invalid ' . $guard . '.';
}
if ($errors) {
