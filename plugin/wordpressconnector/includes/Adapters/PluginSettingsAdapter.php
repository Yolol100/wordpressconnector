...
final class PluginSettingsAdapter
{
    ...
    private const INTEGRATIONS = array(
        ...
        'imagify/imagify.php' => array('id' => 'imagify', 'mode' => 'wordpress_ability', 'actions' => array('plugin.settings.inspect', 'plugin.settings.update', 'wordpress.ability.read', 'wordpress.ability.execute')),
        ...
        'elementor/elementor.php' => array('id' => 'elementor', 'mode' => 'native_adapter', 'actions' => array('elementor.capabilities', 'elementor.inspect', 'elementor.patch_element', 'elementor.replace_document')),
        'elementor-pro-2/elementor-pro.php' => array('id' => 'elementor_pro', 'mode' => 'native_adapter', 'actions' => array('elementor.capabilities', 'elementor.inspect', 'elementor.patch_element', 'elementor.replace_document')),
        ...
        'auto-image-attributes-from-filename-with-bulk-updater/iaff_image-attributes-from-filename.php' => array('id' => 'auto_image_attributes', 'mode' => 'media_adapter_for_individual_images', 'actions' => array('media.get', 'media.update'), 'note' => 'Bulk/global plugin settings remain version-bound until the installed build exposes a stable API.'),
        'broken-link-checker/broken-link-checker.php' => array('id' => 'broken_link_checker', 'mode' => 'version_bound', 'actions' => array(), 'note' => 'Cloud/local engines and account-bound settings require an installed-version API contract before writes are exposed.'),
        'google-site-kit/google-site-kit.php' => array('id' => 'site_kit', 'mode' => 'oauth_bound', 'actions' => array(), 'note' => 'OAuth tokens, service connections and encrypted settings are intentionally not exposed through GitHub requests.'),
        'wordfence/wordfence.php' => array('id' => 'wordfence', 'mode' => 'limited_stable_api', 'actions' => array(), 'note' => 'Generic Wordfence settings writes are withheld because most internals are not a stable public developer API and may contain security secrets.'),
        'wp-mail-smtp/wp_mail_smtp.php' => array('id' => 'wp_mail_smtp', 'mode' => 'secret_bound', 'actions' => array(), 'note' => 'SMTP passwords, OAuth tokens and provider secrets are never exported to GitHub.'),
        ...
        'all-in-one-wp-migration/all-in-one-wp-migration.php' => array('id' => 'all_in_one_wp_migration', 'mode' => 'system_operation', 'actions' => array(), 'note' => 'Backup/import/export operations need a separate high-risk, staging-first contract.'),
        'wp-asset-clean-up/wpacu.php' => array('id' => 'asset_cleanup', 'mode' => 'version_bound', 'actions' => array(), 'note' => 'Asset unload rules can break frontend behavior and need an installed-version contract plus browser readback.'),
    ...
    public function inspect(array $payload, array $context = array()): array
    {
        ...
        $settings = $this->readProfile($plugin);
        $settings = Policy::redact($settings);
    ...
    public function update(array $payload, array $context): array
    {
        ...
        if ($rollbackFields) {
            $result['_rollback'] = array(
                ...
                'payload' => array('plugin' => $plugin, 'fields' => $rollbackFields),
            );
        }
    ...
    {
        $result = $this->executeAbility('imagify/get-settings', array());
        $settings = isset($result['settings']) && is_array($result['settings']) ? $result['settings'] : $result;
