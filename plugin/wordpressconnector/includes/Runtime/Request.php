...
final class Request
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        if(!preg_match('/^[A-Za-z0-9][A-Za-z0-9._-]{7,99}$/',$id)){throw new RuntimeException('request_id must be 8-100 safe characters.');}
        if(!preg_match('/^[a-z0-9][a-z0-9._-]*$/',$action)){throw new RuntimeException('action is invalid.');}
    ...
    private static function hexGuard(array $data,string $key): ?string { if(!array_key_exists($key,$data)||null===$data[$key]){return null;} $value=(string)$data[$key]; if(!preg_match('/^[a-f0-9]{64}$/',$value)){throw new RuntimeException($key.' must be a SHA-256 hex string.');} return $value; }
